import { Experience } from "../types";

export const experienceCardConfig = {
  cardHeightClass: "h-[250px]",
  readMoreLabel: "Read more",
  closeLabel: "Close",
};

export const experienceData: Experience[] = [
  {
    id: "e1",
    role: "Trainee Software Engineer",
    company: "Tetherfi (Pvt) Ltd",
    period: "March 2026 - Present",
    description: [
      "Engineered and optimized a comprehensive PBX Administration UI, delivering an intuitive, highly responsive platform that streamlines complex telecommunications management and system monitoring workflows.",
    ],
  },
  {
    id: "e2",
    role: "Intern Software Engineer",
    company: "BotCalm (Pvt) Ltd",
    period: "July 2024 - November 2025",
    description: [
      "Engineered scalable, full-stack web applications using React.js, Next.js, and Node.js, collaborating with cross-functional teams to deliver secure, high-performing, and responsive user experiences.",
    ],
  },
  {
    id: "e3",
    role: "Demonstrator & Project Preparation",
    company:
      "Faculty of Engineering, University of Ruhuna — ReXtro 2025 Exhibition",
    period: "December 2025",
    description: [
      "Demonstrated the Intelligent Document Information Extraction and Processing Toolkit at the ReXtro 2025 Exhibition.",
      "Prepared and presented the final-year research project to academic and industry audiences.",
      "Explained OCR, RAG pipeline, and knowledge graph integrations in accessible terms to visitors.",
    ],
  },
];
